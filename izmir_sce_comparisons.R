izmir <- read_excel("izmir.xlsx", col_types = c("numeric", "numeric", "numeric", 
...
# for loop
for (i in 1:36){
  scenario1<-plan_izmir %>% slice(i)
  for (j in 1:36){
    scenario2<-plan_izmir %>% slice(j)
    if (scenario1$Blocks==scenario2$Blocks &
        scenario1$Scenario!=scenario2$Scenario) {
      results_izmir <- results_izmir %>% bind_rows(my_function(scenario1,scenario2))
    }
    
  }
}

izmir_output <- results_izmir %>% 
  filter(
    (Pt_situation=="inferior"&Car_situation=="superior")|
      (Pt_situation=="superior"&Car_situation=="inferior")
  ) 

openxlsx::write.xlsx(izmir_output,"izmir_output.xlsx")
